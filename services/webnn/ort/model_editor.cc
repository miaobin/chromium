...
ScopedOrtValueInfo CreateOrtValueInfo(base::cstring_view name,
  ...
  for (const auto& dim : shape) {
    if (std::holds_alternative<webnn::DynamicDimension>(dim)) {
      // A named dynamic dim emits its symbolic name; an unnamed dim emits "",
      // which ORT treats as an anonymous dynamic dimension (`?`).
      const auto& dim_name = std::get<webnn::DynamicDimension>(dim).name;
      dim_params.push_back(dim_name.has_value() ? dim_name->c_str() : "");
