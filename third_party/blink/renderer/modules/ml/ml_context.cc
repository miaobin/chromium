...
void AppendVectorOfNumbers(const std::vector<webnn::Dimension>& vector,
  ...
  for (const auto& dim : vector) {
    ...
    if (std::holds_alternative<uint32_t>(dim)) {
      ...
      builder.Append(String::FromUtf8(dyn.name));
    }
  }
}
...
// dynamic dims must fall within [min_size, max_size].
bool TensorShapeMatchesSpec(const std::vector<uint32_t>& tensor_shape,
  ...
  for (size_t i = 0; i < tensor_shape.size(); ++i) {
    ...
    if (std::holds_alternative<uint32_t>(spec_dim)) {
      ...
      const auto& dyn = std::get<webnn::DynamicDimension>(spec_dim);
      if (tensor_shape[i] < dyn.min_size) {
        return false;
      }
      if (dyn.max_size.has_value() && tensor_shape[i] > dyn.max_size.value()) {
        return false;
      }
    }
  }
