...
std::vector<webnn::Dimension> ToDimensionVector(
  ...
  for (const auto& dimension : dimensions) {
    if (!dimension) {
      // A null element is an unnamed dynamic dimension (ORT's `?`).
      result.push_back(webnn::DynamicDimension{});
    } else if (dimension->IsUnsignedLongEnforceRange()) {
      // A number is a static dimension.
      result.push_back(dimension->GetAsUnsignedLongEnforceRange());
    } else {
      // A string is a named dynamic dimension.
      result.push_back(
          webnn::DynamicDimension{.name = dimension->GetAsString().Utf8()});
    }
  }
  return result;
}

// Returns the static size of a dimension, or nullopt if it is dynamic (named
// or unnamed).
std::optional<uint32_t> GetStaticSize(const Member<MLDimension>& dimension) {
  if (dimension && dimension->IsUnsignedLongEnforceRange()) {
  ...
  auto collect_dims = [&](const MLOperand* operand) {
    for (const auto& dim : operand->Descriptor().shape()) {
      if (std::holds_alternative<webnn::DynamicDimension>(dim)) {
        const auto& name = std::get<webnn::DynamicDimension>(dim).name;
        if (name.has_value()) {
          graph_dynamic_dim_names.insert(String::FromUtf8(*name));
        }
      }
    }
  };
  ...
  for (const MLOperator* current_operator : visited_operators) {
    ...
    for (const auto& dim : expand_output->Descriptor().shape()) {
      if (std::holds_alternative<webnn::DynamicDimension>(dim)) {
        ...
        // Only named dynamic dims must originate from an upstream operand. An
        // unnamed dim is a fresh unknown introduced by the op itself.
        if (dyn_dim.name.has_value() && !graph_dynamic_dim_names.Contains(
                                            String::FromUtf8(*dyn_dim.name))) {
...
void MLGraphBuilder::RegisterOutputDynamicDimensions(
  ...
  for (const webnn::Dimension& dim : descriptor.shape()) {
    if (std::holds_alternative<webnn::DynamicDimension>(dim)) {
      ...
      // Only named dynamic dims are tracked (an unnamed dim has no name to key
      // on and can never be referenced by name from a later op).
      if (dyn.name.has_value()) {
        known_dynamic_dimensions_.insert(String::FromUtf8(*dyn.name), dyn);
      }
    }
  }
}
...
MLOperand* MLGraphBuilder::input(ScriptState* script_state,
  ...
  // Track named dynamic dimensions so later ops (e.g. expand) can reference
  // them by name. A string element is a named dynamic dim; a null element is
  // an unnamed dynamic dim, which is not tracked.
  const auto& shape = desc->shape();
  for (const auto& dimension : shape) {
    if (dimension && dimension->IsString()) {
      const String& dim_name = dimension->GetAsString();
      known_dynamic_dimensions_.insert(
          dim_name, webnn::DynamicDimension{.name = dim_name.Utf8()});
    }
  }
...
MLOperand* MLGraphBuilder::reshape(MLOperand* input,
  ...
  for (const auto& new_dim : new_shape) {
    auto dim = GetStaticSize(new_dim);
...
MLOperand* MLGraphBuilder::range(MLOperand* start,
  ...
  // Use a named dynamic dimension since the output size depends on runtime
  // values of start, limit, delta. The name must be unique per call so
  // multiple range() invocations don't share a symbol.
