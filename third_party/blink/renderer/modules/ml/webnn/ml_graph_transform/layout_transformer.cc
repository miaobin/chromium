...
bool IsDepthwiseConv2d(const MLOperator* conv2d) {
  ...
  // Conv2d channels may be dynamic now that unbounded dynamic shapes are
  // supported. If either channel dim is dynamic, it cannot be proven to be a
  // depthwise conv2d here; fall back to the general path.
  size_t channel_axis;
  ...
  const auto& input_channel_dim = input_shape[channel_axis];
  const auto& output_channel_dim = output_shape[channel_axis];
  if (!std::holds_alternative<uint32_t>(input_channel_dim) ||
      !std::holds_alternative<uint32_t>(output_channel_dim)) {
    return false;
  }
  uint32_t input_channels = std::get<uint32_t>(input_channel_dim);
  uint32_t output_channels = std::get<uint32_t>(output_channel_dim);
