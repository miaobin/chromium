...
OperationPtr CreateExpandOperation(const OperandToIdMap& operand_to_id_map,
  ...
  for (const auto& dim : output->Descriptor().shape()) {
    if (std::holds_alternative<uint32_t>(dim)) {
      ...
      // A named dynamic dim carries its symbolic name; an unnamed dim is a null
      // string.
      expand_mojo->new_shape.push_back(blink_mojom::Dimension::NewDynamicName(
          dyn.name ? String::FromUtf8(*dyn.name) : String()));
    }
  }
