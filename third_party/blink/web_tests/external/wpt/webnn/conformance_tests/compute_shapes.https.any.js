...
if (navigator.ml === undefined) {
  ...
  promise_test(async () => {
    ...
    const input =
        builder.input('input', {dataType: 'float32', shape: ['batch', 3]});
  ...
  promise_test(async () => {
    // A dynamic dimension accepts any positive size; there is no upper bound.
    const context = await getContext();
    const builder = new MLGraphBuilder(context);
    const input =
        builder.input('input', {dataType: 'float32', shape: ['batch', 3]});
    ...
    const shapes = await graph.computeShapes({input: [9, 3]});
    assert_array_equals(shapes.output.shape, [9, 3]);
  }, 'computeShapes resolves a large dynamic dimension (no upper bound)');

  promise_test(async (t) => {
    // A dynamic dimension must resolve to a positive size; 0 is rejected.
    const context = await getContext();
    const builder = new MLGraphBuilder(context);
    const input =
        builder.input('input', {dataType: 'float32', shape: ['batch', 3]});
    ...
    await promise_rejects_js(
        t, TypeError, graph.computeShapes({input: [0, 3]}),
        'a zero-size dynamic dimension rejects');
  }, 'computeShapes rejects a zero-size dynamic dimension');
}
